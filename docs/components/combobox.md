...
The `sl-combobox` works also with local data.

```html preview
<sl-combobox
  class="combobox-local"
  clearable>
</sl-combobox>
...
<script>
  ...
  combobox.source = async (search) => {
      let filter = countries.filter(name => name.toLowerCase().includes(search.toLowerCase())).slice(0, 20);
      return filter.map(d => { return { text: d, value: d } });
  };
</script>
```

[component-metadata:sl-combobox]